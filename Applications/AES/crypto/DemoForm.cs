...
namespace cryptoAES
{
    public partial class DemoForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            // Gets encrypted password from AES
            byte[] encryptPw = AES.getEncryptedPassword();
            string str = Encoding.UTF8.GetString(encryptPw, 0, encryptPw.Length);
            // Output Encrypted Password to GUI
            textBox2.Text = str;

        }
        #endregion

        #region DecryptButton
        private void button3_Click(object sender, EventArgs e)
        {
            // This function decryptes the encrypted password created using AES.encryptPassword()
            byte[] encryptPw = AES.getEncryptedPassword();
            string decryptpw = AES.getDecryptedPassword(encryptPw);
            // Output decrypted password to GUI
            textBox3.Text = decryptpw;
        }
        #endregion
